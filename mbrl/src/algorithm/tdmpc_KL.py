...
class TOLD(nn.Module):
	...
	def __init__(self, cfg):
		super().__init__()
		self.cfg = cfg
		self._enc_backbone, self._enc_mu, self._enc_logvar = h.vae_enc(cfg)
	...
	def h(self, obs):
		z, _, _ = self.encode(obs, sample=False)
		return z
	
	def encode(self, obs, sample=True):
		feat = self._enc_backbone(obs)
		mu = self._enc_mu(feat)
		logvar = self._enc_logvar(feat)
		if sample:
...
class TDMPC():
	...
	def __init__(self, cfg):
		...
		self.beta_duration = cfg.beta_duration
	...
	def update(self, replay_buffer, step):
		...
		z, mu, logvar = self.model.encode(self.aug(obs), sample=True)
		zs = [z.detach()]
		...
		progress = min(1.0, step / self.beta_duration)
		self.beta = self.cfg.beta_start + progress * (self.beta_end - self.cfg.beta_start)
