...
class QValuePixels(nn.Module):
	"""Q-function under pixels observation"""
	def __init__(self,cfg):
		super().__init__()
		encoder_layers = []
		C = int(3*cfg.frame_stack)
		encoder_layers = [NormalizeImg(),
				  ...
				  nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=2), nn.ReLU()]
		out_shape = _get_out_shape((C, cfg.img_size, cfg.img_size), encoder_layers)
		encoder_layers.extend([Flatten(), nn.Linear(np.prod(out_shape), cfg.latent_dim)])
		self.encoder = nn.Sequential(*encoder_layers)

		mlp_layers = [nn.Linear(cfg.latent_dim+cfg.action_dim, cfg.mlp_dim), nn.LayerNorm(cfg.mlp_dim), nn.Tanh(),
						 nn.Linear(cfg.mlp_dim, cfg.mlp_dim), nn.ELU(),
						 nn.Linear(cfg.mlp_dim, 1)]
		self.mlp = nn.Sequential(*mlp_layers)

	def forward(self, s, a):
		encoder_feature = self.encoder(s)
		x = torch.cat([encoder_feature, a], dim=-1)
		return self.mlp(x)
...
def mlp(in_dim, mlp_dim, out_dim,
        act_fn=nn.ELU(),
        residual=False,
        residual_type="plain"):
    ...
    for i, dim in enumerate(mlp_dim):
        layers += [nn.Linear(prev_dim, dim), act_fn]

        if residual and (i % 2 == 1) and (prev_dim == dim):
            if residual_type == "plain":
                layers.append(ResidualBlock(dim, act_fn))
            elif residual_type == "norm":
                layers.append(NormResidual(dim, act_fn))
            elif residual_type == "norm_linear":
                layers.append(NormLinearResidual(dim, act_fn))
            else:
...
class NormResidual(nn.Module):
    ...
    def __init__(self, dim, act_fn, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
    ...
    def forward(self, x):
        r = self.f(x)
        r = r / (r.norm(dim=-1, keepdim=True) + self.eps)
        return x + r

class NormLinearResidual(nn.Module):
...
def q_s(cfg, act_fn=nn.ELU()):
	"""Returns a Q-function that uses Layer Normalization."""
	if cfg.modality == 'pixels': 
		return QValuePixels(cfg)
	else:
		layers = [nn.Linear(cfg.obs_shape[0]+cfg.action_dim, cfg.mlp_dim), nn.LayerNorm(cfg.mlp_dim), nn.Tanh(),
						 nn.Linear(cfg.mlp_dim, cfg.mlp_dim), nn.ELU(),
						 nn.Linear(cfg.mlp_dim, 1)]
		return nn.Sequential(*layers)
...
class ReplayBuffer():
	...
	def update_priorities(self, idxs, priorities):
		self._priorities[idxs] = priorities.squeeze(1).nan_to_num(0).to(self.device) + self._eps
