...
class TOLD(nn.Module):
	...
	def __init__(self, cfg):
		...
		self._dynamics = h.mlp(cfg.latent_dim+cfg.action_dim, cfg.mlp_dim, cfg.latent_dim, residual=True,  residual_type="norm_linear")
		self._reward = h.mlp(cfg.latent_dim+cfg.action_dim, cfg.mlp_dim, 1, residual=True,  residual_type="norm_linear")
		self._pi = h.mlp(cfg.latent_dim, cfg.mlp_dim, cfg.action_dim, residual=True,  residual_type="norm_linear")
...
class TDMPC():
	...
	def __init__(self, cfg):
		...
		self.optim = torch.optim.Adam(
            ...
            lr=self.cfg.lr,
            weight_decay=1e-4 # weight1 
        )
		self.optim_a = torch.optim.Adam(
            self.model.parameters(), 
            lr=self.cfg.lr,
            weight_decay=1e-4  # weight2 
        )
	...
	def update(self, replay_buffer, step):
		...
		current_horizon = int(min(self.cfg.horizon, h.linear_schedule(self.cfg.horizon_schedule, step)))
		obs, next_obses, action, reward, idxs, weights = replay_buffer.sample()

		assert next_obses.size(0) == current_horizon, \
            f"Horizon mismatch: buffer {next_obses.size(0)} vs required {current_horizon}"
		...
		weighted_loss.register_hook(lambda grad: grad * (1/current_horizon))
